...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .container { max-width: 600px; margin: 2.5rem auto; padding: 0 1.5rem; }
    ...
    <div class="card">
        <c:choose>
            <c:when test="${author.id == null}">
                <c:set var="action" value="/authors/save" />
            </c:when>
            <c:otherwise>
                <c:set var="action" value="/authors/update/${author.id}" />
            </c:otherwise>
        </c:choose>

        <form:form method="post" action="${action}" modelAttribute="author">
            ...
            <div class="form-group">
                ...
                <form:textarea path="bio" placeholder="Brief description of the author..." />
            ...
            <div class="btn-row">
                <button type="submit" class="btn-submit">
                    <c:choose>
                        <c:when test="${author.id == null}">Save Author</c:when>
                        <c:otherwise>Update Author</c:otherwise>
                    </c:choose>
                </button>
